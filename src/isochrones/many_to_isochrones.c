...
PGDLLEXPORT Datum many_to_isochrones(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(many_to_isochrones);
...
void process(
    ...
    time_msg("processing pgr_isochrones()",
            start_t, clock());
...
PGDLLEXPORT Datum
many_to_isochrones(PG_FUNCTION_ARGS) {
